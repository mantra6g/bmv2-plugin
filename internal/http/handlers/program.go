...
import (
	...
	api2 "github.com/mantra6g/bmv2-plugin/pkg/api"
	"github.com/mantra6g/bmv2-plugin/pkg/p4compile"
)
...
func (d *Driver) DeployProgramHandler(w http.ResponseWriter, r *http.Request) {
	...
	default:
		...
		if err := json.NewEncoder(w).Encode(api2.ErrorResponse{Error: "method not allowed"}); err != nil {
...
func (d *Driver) deployProgram(w http.ResponseWriter, r *http.Request) {
	var req api2.ProgramDeploymentRequest
	...
	if err != nil {
		...
		if encErr := json.NewEncoder(w).Encode(api2.ErrorResponse{Error: err.Error()}); encErr != nil {
			log.Printf("failed to encode error response: %v", encErr)
		}
		return
	}

	program := &api2.P4Program{P4DeviceConfig: compiled.DeviceConfig, ProgramName: req.P4FileURL, P4Info: compiled.P4Info}
	...
	if err != nil {
		...
		if encErr := json.NewEncoder(w).Encode(api2.ProgramDeploymentResponse{
			Status:  "error",
	...
	if err := json.NewEncoder(w).Encode(api2.ProgramDeploymentResponse{
		Status:      status,
		ProgramName: req.P4FileURL,
		Tables:      api2.GetTableMetadata(program),
		Counters:    api2.GetCounterMetadata(program),
...
func (d *Driver) undeployProgram(w http.ResponseWriter, r *http.Request) {
	...
	if err := d.Switch.ResetPipeline(ctx); err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		if err := json.NewEncoder(w).Encode(api2.ErrorResponse{Error: "failed to reset forwarding pipeline: " + err.Error()}); err != nil {
			log.Printf("failed to encode error response: %v", err)
		}
		return
	}
...
func (d *Driver) GetProgramHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err := json.NewEncoder(w).Encode(api2.P4ProgramResponse{
			Status: "not_deployed",
	...
	if d.CurrentProgram != nil {
		...
		if err := json.NewEncoder(w).Encode(api2.P4ProgramResponse{
			...
			Tables:      api2.GetTableMetadata(d.CurrentProgram),
			Counters:    api2.GetCounterMetadata(d.CurrentProgram),
		}); err != nil {
	...
	w.WriteHeader(http.StatusOK)
	if err := json.NewEncoder(w).Encode(api2.P4ProgramResponse{Status: "not_deployed"}); err != nil {
...
func (d *Driver) VerifyProgramHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := api2.ValidateP4Program(d.CurrentProgram); err != nil {
