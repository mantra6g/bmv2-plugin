...
func (d *Driver) ReadRegistersHandler(w http.ResponseWriter, r *http.Request) {
	...
	regMeta := make(map[uint32]api2.RegisterMetadata)
	if d.CurrentProgram != nil {
		for _, m := range api2.GetRegisterMetadata(d.CurrentProgram) {
			regMeta[m.RegisterID] = m
		}
	}

	// BMv2 does not support wildcard register reads; each register must be read by ID.
	grouped := make(map[uint32][]api2.RegisterValue)
	var readErr string
	for id := range regMeta {
		...
		for {
			...
			for _, entity := range response.GetEntities() {
				...
				grouped[entry.RegisterId] = append(grouped[entry.RegisterId], api2.RegisterValue{
					Index: idx,
	...
	registers := make([]api2.RegisterArrayResponse, 0, len(regMeta))
	for id, meta := range regMeta {
		registers = append(registers, api2.RegisterArrayResponse{
			RegisterID:   id,
	...
	resp := api2.RegistersResponse{Registers: registers}
